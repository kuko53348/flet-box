...
export const createFile = (filePath, content) => {
    try {
        ...
        console.error(`${c('red', '❌')} Failed to create ${filePath}: ${error.message}`);
    }
};
...
export const copyFile = (source, dest) => {
    try {
        if (fs.existsSync(source)) {
            ...
            console.log(`${c('yellow', '⚠️')} Source not found: ${path.basename(source)}`);
            return false;
        }
    } catch (error) {
        console.error(`${c('red', '❌')} Failed to copy ${path.basename(source)}: ${error.message}`);
        return false;
    }
};
...
export const makeExecutable = (filePath) => {
    try {
        ...
        if (process.env.DEBUG) {
            console.error(`${c('red', '⚠️')} Could not make executable: ${path.basename(filePath)}`);
        }
    }
};
