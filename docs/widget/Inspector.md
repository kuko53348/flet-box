...
## Overview
`Inspector` is a ready-to-use building block. Think of it like a LEGO piece: give it some props, place it inside another widget, and FletBox creates the browser element for you.
...
## When to use
Use `Inspector` when you need this kind of interface element. Start with the smallest example, then add one prop at a time. You can copy the example, change the text or color, and see the result immediately.
...
| --- | --- | --- | --- |
| `widget` | `Widget` | - | Property used by the Inspector component. |
| `indent` | `number` | - | Property used by the Inspector component. |
...
```javascript
inspectWidget(container) ; // logs to console
```
