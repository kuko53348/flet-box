...
## Overview
`Radio` is a ready-to-use building block. Think of it like a LEGO piece: give it some props, place it inside another widget, and FletBox creates the browser element for you.
...
## When to use
Use `Radio` when you need this kind of interface element. Start with the smallest example, then add one prop at a time. You can copy the example, change the text or color, and see the result immediately.
...
| --- | --- | --- | --- |
| `width` | `Size` | - | Property used by the Radio component. |
| `height` | `Size` | - | Property used by the Radio component. |
...
| `borderRadius` | `number \| string` | - | Property used by the Radio component. |
| `elevation` | `number` | - | Property used by the Radio component. |
| `shadow` | `string` | - | Property used by the Radio component. |
| `opacity` | `number` | - | Property used by the Radio component. |
| `visible` | `boolean` | false | Property used by the Radio component. |
...
| `onClick` | `(widget: Widget) => void` | - | Property used by the Radio component. |
| `id` | `string` | - | Property used by the Radio component. |
| `className` | `string` | - | Property used by the Radio component. |
| `ref` | `(widget: Widget) => void` | - | Property used by the Radio component. |
| `disableTransform` | `boolean` | - | Property used by the Radio component. |
| `selected` | `boolean` | false | Current selected state. |
| `onSelect` | `(selected: boolean) => void` | - | Callback fired when an option is selected. |
| `activeColor` | `Color` | - | Property used by the Radio component. |
...
```javascript
import { Radio, Text } from "flet-box";
...
```javascript
Radio({ selected: option === "a", onSelect: () => setOption("a"), label: "Option A" })
```
