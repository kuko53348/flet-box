...
## Overview
`QRCode` is a ready-to-use building block. Think of it like a LEGO piece: give it some props, place it inside another widget, and FletBox creates the browser element for you.
...
## When to use
Use `QRCode` when you need this kind of interface element. Start with the smallest example, then add one prop at a time. You can copy the example, change the text or color, and see the result immediately.
...
| --- | --- | --- | --- |
| `width` | `Size` | - | Property used by the QRCode component. |
| `height` | `Size` | - | Property used by the QRCode component. |
...
| `borderRadius` | `number \| string` | - | Property used by the QRCode component. |
| `elevation` | `number` | - | Property used by the QRCode component. |
| `shadow` | `string` | - | Property used by the QRCode component. |
| `opacity` | `number` | - | Property used by the QRCode component. |
| `visible` | `boolean` | false | Property used by the QRCode component. |
...
| `onClick` | `(widget: Widget) => void` | - | Property used by the QRCode component. |
| `id` | `string` | - | Property used by the QRCode component. |
| `className` | `string` | - | Property used by the QRCode component. |
| `ref` | `(widget: Widget) => void` | - | Property used by the QRCode component. |
| `disableTransform` | `boolean` | - | Property used by the QRCode component. |
| `value` | `string` | - | Current value controlled by the widget. |
| `fgColor` | `Color` | - | Property used by the QRCode component. |
| `errorCorrection` | `'L' \| 'M' \| 'Q' \| 'H'` | - | Property used by the QRCode component. |
...
```javascript
import { QRCode, Text } from "flet-box";
...
```javascript
QRCode({ value: userId, size: 200, fgColor: "#000", bgColor: "#fff" })
```
