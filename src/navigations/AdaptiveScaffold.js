...
export const AdaptiveScaffold = (props) => {
    ...
    const isMobile = forceMobile || (!forceDesktop && (device.isMobile() || window.innerWidth < 768));

    let finalLeftNavBar = null;
    let drawerInstance = null;

    if (originalLeftNavBar) {
        const content = typeof originalLeftNavBar === 'function' ? originalLeftNavBar() : originalLeftNavBar;
        ...
        if (isMobile) {
            ...
            if (drawerInstance?.element && !document.body.contains(drawerInstance.element)) {
    ...
    if (!isMobile && originalRightNavBar) {
        const content = typeof originalRightNavBar === 'function' ? originalRightNavBar() : originalRightNavBar;
