...
export const initHMR = () => {
    ...
    ws.onmessage = (event) => {
        ...
        if (data.type === 'request-modules') {
            ...
            ws.send(JSON.stringify({ type: 'register-modules', modules: moduleUrls }));
        }
...
const handleHMR = (data) => {
    ...
    if (changeType === 'css') {
        ...
        const links = document.querySelectorAll(`link[href*="${file.replace(/\\/g, '/')}"]`);
        links.forEach(link => {
    ...
    import(url).then((module) => {
        ...
        window.dispatchEvent(new CustomEvent('hmr:update', { detail: { file, module } }));
    }).catch((err) => {
