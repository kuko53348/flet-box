...
export const filter = (options = {}) => {
    ...
    if (options.blur) parts.push(`blur(${typeof options.blur === 'number' ? options.blur + 'px' : options.blur})`);
    ...
    if (options.backdrop) parts.push(`backdrop-filter(${filter(options.backdrop)})`);
