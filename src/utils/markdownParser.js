...
const isTableSeparatorLine = (line) => {
    return line.includes('|') && line.includes('-') && /[\s\|]*:?-+:?[\s\|]*/.test(line);
};
...
export const parseInlineToWidgets = (text) => {
    ...
    return [Row({ alignItems: 'center', gap: 4, children: parts, flexWrap: 'wrap' })];
};
...
export const markdownToWidgets = (text, options = {}) => {
    ...
    while (i < lines.length) {
        ...
        // HEADERS
        if (line.match(/^### /)) {
            widgets.push(Text({ text: line.substring(4), size: 18, weight: 'bold', marginTop: 16, marginBottom: 8 }));
            i++;
            continue;
        }
        if (line.match(/^## /)) {
            widgets.push(Text({ text: line.substring(3), size: 22, weight: 'bold', marginTop: 20, marginBottom: 10 }));
            i++;
            continue;
        }
        if (line.match(/^# /)) {
            widgets.push(Text({ text: line.substring(2), size: 28, weight: 'bold', marginTop: 24, marginBottom: 12 }));
            i++;
            continue;
        }
        ...
        if (line.match(/^---$/) || line.match(/^\*\*\*$/)) {
            widgets.push(Container({ height: 1, bgColor: colors.border, marginVertical: 16 }));
            i++;
            continue;
        }
        ...
        if (line.match(/^\s*[-*+]\s/)) {
            ...
            widgets.push(Row({
                ...
                gap: 8,
                marginVertical: 4,
                children: [Icon({ name: 'circle', size: 8, color: colors.primary }), ...innerWidgets]
            }));
            i++;
            continue;
        }
        ...
        if (line.match(/^\s*\d+\.\s/)) {
            ...
            widgets.push(Row({
                ...
                gap: 8,
                marginVertical: 4,
                children: [Text({ text: `${number}.`, size: 14, weight: 'bold', color: colors.primary }), ...innerWidgets]
            }));
            i++;
            continue;
        }
        ...
        if (paragraphWidgets.length === 1) {
            ...
            widgets.push(Row({ alignItems: 'center', gap: 4, children: paragraphWidgets, flexWrap: 'wrap', marginVertical: 4 }));
        }
        i++;
    }
