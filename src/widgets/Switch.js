...
export const Switch = (props) => {
    ...
    const updateUI = (newValue) => {
        if (switchRef) {
            switchRef.style.backgroundColor = newValue ? colors.success : colors.gray300;
        }
        if (knobRef) {
            knobRef.style.transform = newValue ? `translateX(${sz.width - sz.knob - 4}px)` : 'translateX(0)';
        }
    };
    ...
    const knob = WidgetFactory({
        ...
        borderRadius: '50%',
        transform: internalValue ? `translateX(${sz.width - sz.knob - 4}px)` : 'translateX(0)',
