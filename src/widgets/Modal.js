...
const ModalOverlay = ({ closeOnOverlayClick, onClose, zIndex = 9998, overlayColor }) => {
...
const ModalHeader = ({ 
    ...
    const finalBorder = headerBorder === false ? 'none' : (headerBorder || defaultBorder);
    
    // Optional elevation shadow for header
    const headerShadow = headerElevation ? `0 ${headerElevation}px ${headerElevation * 2}px rgba(0,0,0,0.05)` : 'none';
...
const ModalContent = ({ content, padding, contentBgColor, contentElevation }) => {
    const wrapper = Container({
        style: {
            ...
            boxShadow: contentElevation ? `inset 0 ${contentElevation}px ${contentElevation * 2}px rgba(0,0,0,0.02)` : 'none'
        }
    });
    ...
    if (content) {
        if (content instanceof HTMLElement) {
            ...
            wrapper.appendChild(Text({ text: content, size: 14, color: colors.text }));
        } else if (Array.isArray(content)) {
            content.forEach(item => {
                if (item instanceof HTMLElement) wrapper.appendChild(item);
                else if (typeof item === 'string') wrapper.appendChild(Text({ text: item, size: 14, color: colors.text }));
            });
        }
    }
    return wrapper;
...
const ModalFooter = ({ actions, footerBgColor, footerBorder, footerPadding, footerElevation }) => {
    if (!actions || !actions.length) return null;
    
    const defaultBorder = `1px solid ${colors.border}`;
    const finalBorder = footerBorder === false ? 'none' : (footerBorder || defaultBorder);
    const footerShadow = footerElevation ? `0 -${footerElevation}px ${footerElevation * 2}px rgba(0,0,0,0.05)` : 'none';
...
const ModalContainer = ({ 
    width, minWidth, maxWidth, maxHeight, 
    ...
    const finalShadow = shadow || (elevation ? elevationShadows[elevation] : elevationShadows[3]);
    
    let containerStyle = {
        display: 'flex',
        flexDirection: 'column',
        backgroundColor: backgroundColor || colors.surface,
        borderRadius: typeof borderRadius === 'number' ? `${borderRadius}px` : (borderRadius || '16px'),
...
export const Modal = (props = {}) => {
    ...
    const modalChildren = [headerElement, contentElement, footerElement].filter(Boolean);
    ...
    const instance = {
        ...
        updateContent: (newContent) => {
            if (contentElement) {
                ...
                if (newContent instanceof HTMLElement) {
                    ...
                    contentElement.appendChild(Text({ text: newContent, size: 14, color: colors.text }));
                }
            }
        },
        
        updateTitle: (newTitle) => {
            const titleText = modalBody?.querySelector('.modal-title') || 
        ...
        updatePadding: (newPadding) => {
            if (contentElement) {
                contentElement.style.padding = typeof newPadding === 'number' ? `${newPadding}px` : newPadding;
            }
        },
                ...
                const loadingEl = Container({
                    ...
                    child: Column({
                        ...
                        children: [
                            ...
                            Text({ text: loadingText, size: 14, color: colors.textSecondary })
                        ]
                    })
                });
