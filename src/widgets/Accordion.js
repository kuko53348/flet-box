...
export const Accordion = (props) => {
    ...
    const updateUI = () => {
        ...
        // Estilos del título
        titleElement.style.color = isExpanded ? currentExpandedColor : currentTitleColor;
        titleElement.style.fontSize = `${currentTitleSize}px`;
        titleElement.style.fontWeight = currentTitleWeight;

        // Icono
        iconElement.setAttribute('name', isExpanded ? currentIconExpanded : currentIconCollapsed);
        ...
        if (currentVariant === 'contained') {
            titleBar.style.backgroundColor = isExpanded ? `${currentExpandedColor}10` : 'transparent';
        } else {
        ...
        if (typeof ResizeObserver !== 'undefined') {
            resizeObserver = new ResizeObserver(() => {
                if (isExpanded && contentWrapper && !isAnimating) {
                    ...
                    if (contentWrapper.style.height !== 'auto' && contentWrapper.style.height !== newHeight) {
                        setHeight(newHeight);
    ...
    iconElement = Icon({
        ...
        style: { transition: animate ? `transform ${currentAnimationDuration}ms ease` : 'none' }
    });
    ...
    titleBar = Row({
        ...
        style: {
            ...
            backgroundColor: (isExpanded && currentVariant === 'contained') ? `${currentExpandedColor}10` : 'transparent'
        },
    ...
    contentWrapper = WidgetFactory({
        ...
        style: {
            overflow: 'hidden',
            transition: animate ? `height ${currentAnimationDuration}ms cubic-bezier(0.4, 0, 0.2, 1)` : 'none',
    ...
    contentInner = Container({
        style: {
            ...
            borderTop: (currentDivider && isExpanded) ? `1px solid ${currentDividerColor}` : 'none'
        },
    ...
    const update = (newProps) => {
        ...
        if (newProps.children !== undefined) {
            ...
            while (contentInner.firstChild) contentInner.removeChild(contentInner.firstChild);
            const childrenArray = Array.isArray(currentChildren) ? currentChildren : [currentChildren];
            childrenArray.forEach(child => {
                if (child instanceof HTMLElement) contentInner.appendChild(child);
                else if (typeof child === 'string') contentInner.appendChild(document.createTextNode(child));
            });
        ...
        if (newProps.expandedColor !== undefined) currentExpandedColor = newProps.expandedColor;
        if (newProps.titleColor !== undefined) currentTitleColor = newProps.titleColor;
        if (newProps.titleSize !== undefined) currentTitleSize = newProps.titleSize;
        if (newProps.titleWeight !== undefined) currentTitleWeight = newProps.titleWeight;
        if (newProps.titlePadding !== undefined) currentTitlePadding = newProps.titlePadding;
        if (newProps.contentPadding !== undefined) {
            currentContentPadding = newProps.contentPadding;
            contentInner.style.padding = currentContentPadding;
        }
        if (newProps.iconCollapsed !== undefined) currentIconCollapsed = newProps.iconCollapsed;
        if (newProps.iconExpanded !== undefined) currentIconExpanded = newProps.iconExpanded;
        ...
        if (newProps.dividerColor !== undefined) currentDividerColor = newProps.dividerColor;
        if (newProps.borderRadius !== undefined) currentBorderRadius = newProps.borderRadius;
        if (newProps.border !== undefined) currentBorder = newProps.border;
        if (newProps.borderColor !== undefined) currentBorderColor = newProps.borderColor;
        if (newProps.borderWidth !== undefined) currentBorderWidth = newProps.borderWidth;
        if (newProps.elevation !== undefined) currentElevation = newProps.elevation;
        if (newProps.animationDuration !== undefined) currentAnimationDuration = newProps.animationDuration;
