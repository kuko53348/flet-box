...
injectKeyframes('slider-stripes', 
    `0% { background-position: 0 0; } 100% { background-position: 20px 0; }`
);
injectKeyframes('slider-glow', 
...
export const Slider = (props) => {
    let {
        // Valores básicos
        value = 0, min = 0, max = 100, step = 1, disabled = false,
        // Apariencia
        width = '100%', height = 4, thumbSize = 20,
        color = colors.primary, trackColor = colors.border,
        thumbColor = '#fff',
        orientation = 'horizontal', inverted = false,
        // Texto y marcas
        showValue = false, valuePrefix = '', valueSuffix = '',
        showMarks = false, marks = [],           // ← implementado
        // Efectos visuales
        striped = true, animatedStripes = false, stripeColor = 'rgba(255,255,255,0.25)',
        glow = false,
        // Eventos
    ...
    let trackLengthPx = typeof trackSizePx === 'number' ? `${trackSizePx}px` : trackSizePx;
    ...
    function renderMarks() {
        ...
        while (marksContainer.firstChild) marksContainer.removeChild(marksContainer.firstChild);
