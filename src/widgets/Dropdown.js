...
export const Dropdown = (props) => {
    ...
    const findSelected = () => currentOptions.find(opt => getValue(opt) === selectedValue);
    ...
    const selector = Row({
        ...
        style: {
            ...
            backgroundColor: currentVariant === 'filled' ? `${currentBgColor}CC` : 'transparent',
            border: currentVariant === 'outlined' ? `1px solid ${currentError ? colors.danger : colors.border}` : 'none',
    ...
    const openMenu = () => {
        ...
        currentOptions.forEach(opt => {
            ...
            const leftOption = [];
            if (optIcon) leftOption.push(Text({ text: optIcon, size: 16, color: currentTextColor }));
            leftOption.push(Text({ text: optLabel, size: currentSize.fontSize, style: { flex: 1 }, color: currentTextColor }));
            ...
            const optionItem = Row({
                ...
                children: [
                    ...
                    isSelected ? Icon({ name: 'check', size: 16, color: currentColor }) : null
                ].filter(Boolean)
            });

            optionItem.onmouseenter = () => {
                if (!isSelected) optionItem.style.backgroundColor = currentOptionHoverColor;
            };
            optionItem.onmouseleave = () => {
                optionItem.style.backgroundColor = isSelected ? currentColor + '15' : 'transparent';
            };
        ...
        outsideClickListener = (e) => {
            if (!container.contains(e.target) && (!menuElement || !menuElement.contains(e.target))) {
                closeMenu();
            }
        };
    ...
    const update = (newProps) => {
        ...
        if (newProps.options !== undefined) { currentOptions = [...newProps.options]; needsDisplayUpdate = true; }
        if (newProps.value !== undefined && newProps.value !== selectedValue) {
            selectedValue = newProps.value;
            needsDisplayUpdate = true;
            if (onChange) onChange(selectedValue);
        }
        if (newProps.disabled !== undefined) { currentDisabled = newProps.disabled; needsDisplayUpdate = true; if (currentDisabled && isOpen) closeMenu(); }
        if (newProps.error !== undefined) { currentError = newProps.error; needsDisplayUpdate = true; }
        if (newProps.variant !== undefined) { currentVariant = newProps.variant; needsDisplayUpdate = true; }
        if (newProps.placeholder !== undefined) { currentPlaceholder = newProps.placeholder; needsDisplayUpdate = true; }
        if (newProps.label !== undefined) {
            ...
            if (existingLabel) existingLabel.textContent = currentLabel;
                ...
                const labelEl = WidgetFactory({ tag: 'label', textContent: currentLabel, fontSize: '12px', color: currentError ? colors.danger : colors.textSecondary, marginBottom: '4px', display: 'block' });
                container.insertBefore(labelEl, container.firstChild);
            }
        }
        if (newProps.clearable !== undefined) { currentClearable = newProps.clearable; needsDisplayUpdate = true; }
        if (newProps.color !== undefined) { currentColor = newProps.color; needsDisplayUpdate = true; }
        if (newProps.bgColor !== undefined) { currentBgColor = newProps.bgColor; needsDisplayUpdate = true; }
        if (newProps.textColor !== undefined) { currentTextColor = newProps.textColor; needsDisplayUpdate = true; }
        if (newProps.borderRadius !== undefined) { currentBorderRadius = newProps.borderRadius; needsDisplayUpdate = true; }
        if (newProps.optionHoverColor !== undefined) { currentOptionHoverColor = newProps.optionHoverColor; }
        if (newProps.size !== undefined && sizes[newProps.size]) { currentSize = sizes[newProps.size]; needsDisplayUpdate = true; }
        if (needsDisplayUpdate) {
            updateDisplay();
            selector.style.padding = currentSize.padding;
            selector.style.borderRadius = `${currentBorderRadius}px`;
            if (currentVariant === 'outlined') selector.style.border = `1px solid ${currentError ? colors.danger : colors.border}`;
            else selector.style.border = 'none';
            selector.style.backgroundColor = currentVariant === 'filled' ? `${currentBgColor}CC` : 'transparent';
        }
    };

    Object.defineProperty(container, 'value', { get: () => selectedValue, set: (v) => update({ value: v }) });
    Object.defineProperty(container, 'options', { get: () => currentOptions, set: (opts) => update({ options: opts }) });
    Object.defineProperty(container, 'disabled', { get: () => currentDisabled, set: (d) => update({ disabled: d }) });
    Object.defineProperty(container, 'error', { get: () => currentError, set: (e) => update({ error: e }) });
