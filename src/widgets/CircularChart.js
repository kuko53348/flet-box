...
export const CircularChart = (props) => {
    let {
        ...
        onClick = null,
        onHover = null,
        defaultColors = [colors.primary, colors.secondary, colors.success, colors.warning, colors.danger, colors.info],
        ref,
        shadowBlur = 0,
    ...
    const getCenterX = () => {
        ...
        switch (semiCircle) {
            case 'left': return currentWidth;
            case 'right': return 0;
            default: return currentWidth / 2;
        }
    };
    const getCenterY = () => {
        if (!isSemi) return currentHeight / 2;
        switch (semiCircle) {
            case 'top': return currentHeight;
            case 'bottom': return 0;
            default: return currentHeight / 2;
        }
    };
    ...
    const drawRoundedSegment = (slice, index) => {
        ...
        if (showLabels && slice.value > 0) {
            ...
            ctx.shadowBlur = 0;
            const percent = Math.round((slice.value / (slice.end - slice.start)) * 100);
    ...
    const drawCenterContent = () => {
        ...
        if (isSemi) {
            const r = getRadiusVal() / 2;
            switch (semiCircle) {
                case 'top': cy = cy - r * 0.3; break;
                case 'bottom': cy = cy + r * 0.3; break;
                case 'left': cx = cx - r * 0.3; break;
                case 'right': cx = cx + r * 0.3; break;
            }
        }
    ...
    const updateAnimation = (timestamp) => {
        if (!ctx || !canvas) return;
        if (!startTime) { startTime = timestamp; requestAnimationFrame(updateAnimation); return; }
    ...
    const handleCanvasMousemove = (e) => {
        ...
        if (hoveredIndex !== idx) {
            hoveredIndex = idx;
            if (onHover) onHover(idx !== -1 ? currentSlices[idx]?.original : null, idx);
            draw();
        }
    };
