...
export const CircularBar = (props) => {
  ...
  const getFormattedValue = () => {
    ...
    if (valueFormat === "percent") {
      const percent =
        Math.round((currentValue / max) * 100 * Math.pow(10, valueDecimals)) /
        Math.pow(10, valueDecimals);
      return `${valuePrefix}${percent}${valueSuffix}`;
    }
    if (valueFormat === "value") {
      const val =
        Math.round(currentValue * Math.pow(10, valueDecimals)) /
        Math.pow(10, valueDecimals);
  ...
  const drawText = () => {
    ...
    if (label) {
      ...
      ctx.fillText(
        label,
        getCenter(),
        getCenter() + valueSize / 1.5 + (subtitle ? 4 : 0),
      );
    }
    if (subtitle) {
      ...
      ctx.fillText(
        subtitle,
        getCenter(),
        getCenter() + valueSize / 1.5 + (label ? 14 : 4),
      );
    }
  };
    ...
    if (onClick)
      canvas.addEventListener("click", (e) =>
        onClick({
          value: currentValue,
          max,
          percent: (currentValue / max) * 100,
        }),
      );
    if (onHover) {
      canvas.addEventListener("mouseenter", () => {
        isHovering = true;
        onHover(true, { value: currentValue, max });
      });
