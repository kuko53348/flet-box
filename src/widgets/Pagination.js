...
export const Pagination = (props) => {
    ...
    const createIconButton = (iconName, onClick, isDisabled = false) => {
        return Button({
            child: Icon({ name: iconName, size: sz.iconSize, color: colors.textSecondary }),
            variant: variant,
            size: size,
            style: buttonStyle,
            onPress: onClick,
    ...
    if (showFirstLast && totalPages > 1) {
        container.appendChild(createIconButton('first_page', () => handlePageChange(1), current === 1));
    }

    // Previous button
    if (showPrevNext) {
        container.appendChild(createIconButton('chevron_left', () => handlePageChange(current - 1), current === 1));
    }
    ...
    if (pageNumbers[0] > 1) {
        ...
        if (pageNumbers[0] > 2) {
            container.appendChild(Text({ text: '...', size: sz.fontSize, color: colors.textSecondary }));
        }
    }
    ...
    // Next button
    if (showPrevNext) {
        container.appendChild(createIconButton('chevron_right', () => handlePageChange(current + 1), current === totalPages));
    }

    // Last button
    if (showFirstLast && totalPages > 1) {
        container.appendChild(createIconButton('last_page', () => handlePageChange(totalPages), current === totalPages));
    }
